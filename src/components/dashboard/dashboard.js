...
const StockSummary = () => {
  ...
  const bestSellingProducts = showMore ? [...initialProducts, ...additionalProducts] : initialProducts;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-red-800 shadow-md rounded-lg p-6">
          ...
          <p className="text-3xl font-bold">
            {showPercentage ? `${((weeklySales / totalSales) * 100).toFixed(2)}%` : weeklySales}
          </p>
          <button
          ...
          <ul className="mb-4">
            {bestSellingProducts.map((product, index) => (
              <li key={index} className="mb-2">
                <span className="font-medium">{product.name}</span>: {product.sales} sales
              </li>
            ))}
          </ul>
          <button
